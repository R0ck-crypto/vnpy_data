...
from vnpy.trader.object import TickSubscribeRequest, TradeSubscribeRequest, DepthSubscribeRequest, KlineSubscribeRequest
...
class StrategyEngine(BaseEngine):
    ...
    def _register_event(self) -> None:
        ...
        self.event_engine.register(EVENT_DEPTH_BINANCE, self._process_depth_event)
    ...
    def _process_depth_event(self, event: Event) -> None:
        """Process tick data event"""
        depth:BinanceDepthData = event.data
        symbol = depth.symbol + '.' + 'BINANCE'
        ...
        for strategy in strategies:
            if strategy.inited:
                self._call_strategy_func(strategy, strategy.on_depth, depth)
    ...
    def _init_strategy(self, strategy_name: str) -> None:
        ...
        for vt_symbol in strategy.vt_symbols:
            contract: Optional[ContractData] = self.main_engine.get_contract(vt_symbol)
            if contract:
                if strategy.subscribe_ticker:
                    req = TickSubscribeRequest(symbol=contract.symbol, exchange=contract.exchange)
                    self.main_engine.subscribe(req, contract.gateway_name)
                    self.write_log(f"Subscribed to ticker data for {vt_symbol}", strategy)

                if strategy.subscribe_trade:
                    req = TradeSubscribeRequest(symbols=[contract.symbol], exchange=contract.exchange)
                    self.main_engine.subscribe(req, contract.gateway_name)
                    self.write_log(f"Subscribed to trade data for {vt_symbol}", strategy)

                if strategy.subscribe_depth:
                    depth_level = strategy.depth_level if strategy.depth_level else 5
                    req = DepthSubscribeRequest(symbols=[contract.symbol], exchange=contract.exchange, level=depth_level)
                    self.main_engine.subscribe(req, contract.gateway_name)
                    self.write_log(f"Subscribed to depth data for {vt_symbol} with level {depth_level}", strategy)

                if strategy.subscribe_kline:
                    req = KlineSubscribeRequest(symbols=[contract.symbol], exchange=contract.exchange)
                    self.main_engine.subscribe(req, contract.gateway_name)
                    self.write_log(f"Subscribed to kline data for {vt_symbol}",strategy)
            else:
                self.write_log(f"Market data subscription failed, contract not found: {vt_symbol}", strategy)
